...
def getPatchFromSIFTParameters(position, scale, orient, im):
	...
	rot = np.array([[np.cos(angle-np.pi/2), np.sin(angle-np.pi/2)],
	...
	print(tlr.shape)
	poly_verts.append((tlr[0][0][0], tlr[0][1][0]))
	poly_verts.append((trr[0][0][0], trr[0][1][0]))
	poly_verts.append((brr[0][0][0], brr[0][1][0]))
	poly_verts.append((blr[0][0][0], blr[0][1][0]))
	...
	xmin = int(max(xmin, 0))
	xmax = int(min(xmax, im.shape[1]))
	ymin = int(max(ymin, 0))
	ymax = int(min(ymax, im.shape[0]))
